...
[![tests](https://img.shields.io/github/actions/workflow/status/blueion0612/sEMG_Gesture_Classifier/tests.yml?branch=main&label=tests)](https://github.com/blueion0612/sEMG_Gesture_Classifier/actions/workflows/tests.yml)
...
[**Dataset**](docs/dataset.md) · [**Data DOI**](https://doi.org/10.5061/dryad.8sf7m0czv) · [**Source paper**](https://doi.org/10.1038/s41597-024-04296-8) · [**Related**](#related)
...
*The four evaluations, easiest at the top. Each rung holds out more of what the
training set shares with the test set. The hero is the structure and not a result
because no numbers are committed here; `semg.pipeline` writes them.*
...
```bash
git clone https://github.com/blueion0612/sEMG_Gesture_Classifier
cd sEMG_Gesture_Classifier
pip install -e .
python -m semg.pipeline        # also installed as the semg-pipeline command
```
...
128 ms windows, 256 samples at 2 kHz, with a 50 ms hop. That yields
...
Hudgins-4 is mean absolute value, zero crossings, slope sign
...
### Counting features need a threshold

Zero crossings, slope sign changes and
...
Decision tree, random forest, AdaBoost, histogram gradient boosting, KNN,
...
docs/
  dataset.md      source, license, layout, recording protocol
  figures/        README figure, the script that draws it, figstyle.py
pyproject.toml    package definition, dependencies, the semg-pipeline command
```
...
```bash
python -m pytest -q
python tests/test_features.py   # without pytest
```
...
- **Position transfer is the failure case**, and the dataset is built to expose it:
  only the center position is shared between the two days.
...
- **The report that accompanied this code contains extended tables** that a separate
  run produced, including a training-time benchmark and a subject-wise normalization
...
Cite the dataset and its paper rather than this repository. The repository itself
is described by `CITATION.cff`, version 1.0.0, if a reference to the code is needed:
...
MIT. See [LICENSE](LICENSE). The dataset carries its own license, CC0 1.0.
